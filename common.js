...
function siblings(increment2) {
  ...
  function sibling(parent, index, includeWhitespace) {
    ...
    let offset3 = (index || 0) + increment2;
    let next = siblings2[offset3];
    if (!includeWhitespace) {
      while (next && whitespace(next)) {
        offset3 += increment2;
        next = siblings2[offset3];
      }
    }
    return next;
  }
...
function splitToken(token, offsets) {
  ...
  for (const offset3 of offsets) {
    if (offset3 > lastOffset) {
      tokens.push({
        ...token,
        content: token.content.slice(lastOffset, offset3),
  ...
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    for (let j = 0; j < tokensLength; j++) {
      ...
      if (options.includeExplanation) {
        ...
        token.explanation = [];
        let offset3 = 0;
        while (startIndex + offset3 < nextStartIndex) {
          const tokenWithScopes = tokensWithScopes[tokensWithScopesIndex];
          const tokenWithScopesText = line.substring(
            tokenWithScopes.startIndex,
            tokenWithScopes.endIndex
          );
          offset3 += tokenWithScopesText.length;
...
var DEFAULT_COLOR_LIGHT_DARK, COLOR_KEYS, _grammarStateMap, GrammarState, builtInTransformers, namedColors, decorations, defaultNamedColorsMap, defaultAnsiColors, hastToHtml, VSCODE_FALLBACK_EDITOR_FG, VSCODE_FALLBACK_EDITOR_BG, RESOLVED_KEY, _emitDeprecation, _emitError, ShikiError2, Registry2, Resolver, instancesCount;
var init_dist3 = __esm({
  "node_modules/@shikijs/core/dist/index.mjs"() {
    ...
    DEFAULT_COLOR_LIGHT_DARK = "light-dark()";
    COLOR_KEYS = ["color", "background-color"];
...
var ShikiError3, alignUp, __defProp2, __defNormalProp, __publicField, onigBinding, UtfString, _OnigString, OnigString, OnigScanner, initPromise;
var init_dist4 = __esm({
  "node_modules/@shikijs/engine-oniguruma/dist/index.mjs"() {
    ...
    _OnigString = class _OnigString2 {
      constructor(str) {
    ...
    OnigScanner = class {
      ...
      _findNextMatchSync(string, startPosition, debugCall, options) {
        ...
        let offset3 = resultPtr / 4;
        const index = HEAPU32[offset3++];
        const count = HEAPU32[offset3++];
        const captureIndices = [];
        for (let i = 0; i < count; i++) {
          const beg = string.convertUtf8OffsetToUtf16(HEAPU32[offset3++]);
          const end = string.convertUtf8OffsetToUtf16(HEAPU32[offset3++]);
          captureIndices[i] = {
            start: beg,
            end,
            length: end - beg
  ...
  window.sprocketHighlighterPromise = (async () => {
    try {
      ...
      const highlighter = await createHighlighterCore({
        themes: [material_theme_ocean_default],
...
try {
  ...
  for (const codeElem of document.querySelectorAll("pre > code.language-wdl")) {
    ...
    codeElem.parentElement.outerHTML = `
...
function getDocumentRect(element2) {
  const html5 = getDocumentElement(element2);
  const scroll = getNodeScroll(element2);
  const body3 = element2.ownerDocument.body;
  const width = max(html5.scrollWidth, html5.clientWidth, body3.scrollWidth, body3.clientWidth);
  const height = max(html5.scrollHeight, html5.clientHeight, body3.scrollHeight, body3.clientHeight);
  let x = -scroll.scrollLeft + getWindowScrollBarX(element2);
  const y = -scroll.scrollTop;
  if (getComputedStyle2(body3).direction === "rtl") {
    x += max(html5.clientWidth, body3.clientWidth) - width;
  }
  return {
    width,
    height,
...
function getViewportRect(element2, strategy) {
  ...
  const windowScrollbarX = getWindowScrollBarX(html5);
  if (windowScrollbarX <= 0) {
    const doc = html5.ownerDocument;
    const body3 = doc.body;
    const bodyStyles = getComputedStyle(body3);
    const bodyMarginInline = doc.compatMode === "CSS1Compat" ? parseFloat(bodyStyles.marginLeft) + parseFloat(bodyStyles.marginRight) || 0 : 0;
    const clippingStableScrollbarWidth = Math.abs(html5.clientWidth - body3.clientWidth - bodyMarginInline);
...
var SprocketTooltip = class extends HTMLElement {
  ...
  _updatePosition() {
    ...
    computePosition2(this, tooltipElement, {
      placement: position,
      middleware: [
        offset2(8),
        flip2(),
