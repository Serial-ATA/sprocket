...
pub fn code_lens(graph: &DocumentGraph, document_uri: &Url) -> Result<Option<Vec<CodeLens>>> {
    ...
    for task in analysis_doc.tasks() {
        if task.inputs().values().any(|i| i.required()) {
            continue;
        }

        let start_offset = TextSize::from(task.name_span().start() as u32);
        let end_offset = TextSize::from(task.name_span().end() as u32);
    ...
    if let Some(wf) = analysis_doc.workflow()
        && wf.inputs().values().all(|i| !i.required())
    {
        let start_offset = TextSize::from(wf.name_span().start() as u32);
        let end_offset = TextSize::from(wf.name_span().end() as u32);
