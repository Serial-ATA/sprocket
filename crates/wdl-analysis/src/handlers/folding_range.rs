...
struct FoldingContext {
    ...
    visited_elements: HashSet<usize>,
}
...
fn determine_folding_range(
    ...
    if ctx.visited_elements.contains(&element.index()) {
        return None;
    }
    ...
    match element {
        SyntaxElement::Token(token) => {
            ...
            ctx.visited_elements.insert(comment.inner().index());
            ...
            for sibling in comment.inner().siblings_with_tokens(Direction::Next) {
                ...
                ctx.visited_elements.insert(sibling_comment.inner().index());
            }
        }
        ...
        match sibling {
            ...
            SyntaxElement::Node(node) => {
                if node.kind() == first.kind() {
                    ctx.visited_elements.insert(node.index());
