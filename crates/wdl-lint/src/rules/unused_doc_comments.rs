...
use wdl_analysis::{Diagnostics, Visitor};
use wdl_ast::{
    AstToken, Comment, Diagnostic, Span, SyntaxElement, SyntaxKind, TreeNode, TreeToken,
};
...
fn unused_doc_comment_diagnostic(span: Span, syntax_kind: SyntaxKind) -> Diagnostic {
    Diagnostic::note(format!(
        // TODO: Is it worth including the SyntaxKind here? Is there better information to include?
...
/// Detects whether a doc comment has been placed atop a Node that we do not generate
/// documentation for.
...
impl Rule for UnusedDocCommentsRule {
    ...
    fn explanation(&self) -> &'static str {
        "Some syntax nodes are not supported by doc comments (`##`). This lint reports if a doc comment \
        is attached to syntax elements that aren't supported.
...
impl Visitor for UnusedDocCommentsRule {
    ...
    fn comment(&mut self, diagnostics: &mut Diagnostics, comment: &Comment) {
        ...
        // If the visited comment isn't a doc comment, or we've already seen it, then there's no need to process it!
        ...
        while let Some(sibling) = current {
            ...
            // Given PreambleCommentPlacementRule currently handles "floating comments", we check
            // for them here as well and don't lint on them (as they are assumed to be misplaced preambles).
            ...
            // TODO: This feels pretty jank. Is there a better way to target a field of a struct?
