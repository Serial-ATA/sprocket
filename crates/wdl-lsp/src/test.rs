...
/// An individual test definition under an entrypoint.
...
/// The tests within a [`SprocketTestYaml`] file.
#[derive(Clone, Debug, Deserialize)]
#[serde(transparent)]
pub struct DocumentTests {
    /// A map of the test entrypoints to their sub-tests.
    pub entrypoints: indexmap::IndexMap<Spanned<String>, Vec<TestDefinition>>,
}
...
pub struct SprocketTestYaml {
    ...
    pub path: PathBuf,
    /// The parsed tests, if any.
    pub tests: Option<DocumentTests>,
}
...
impl SprocketTestCache {
    ...
    pub async fn ensure_parsed(&self, uri: Url) -> Result<Option<Arc<SprocketTestYaml>>> {
        ...
        if let Entry::Occupied(mut entry) = docs.entry(uri) {
            if entry.get().tests.is_none() {
                let test_yaml = Arc::make_mut(entry.get_mut());
                test_yaml.tests = Some(marked_yaml::from_yaml(0, &test_yaml.source)?);
            }
